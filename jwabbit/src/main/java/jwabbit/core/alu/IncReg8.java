 ...
 * The "inc_reg8" opcode.
 */
public final class IncReg8 implements IOpcode {

    /**
     * Constructs a new {@code IncReg8}.
     */
    public IncReg8() {
     ...
     *
     * <p>
     * WABBITEMU SOURCE: core/alu.c, "inc_reg8" function.
     */
    @Override
    public int exec(final CPU cpu) {
        ...
        cpu.setF(CPU.signchk(result) + CPU.zerochk(result) + CPU.x5chk(result)
                + CPU.hcaddchk(reg, 1, 0) + CPU.x3chk(result) + CPU.vchkadd(reg, 1, result)
                + CPU.ADD_INSTR + cpu.unaffect(JWCoreConstants.CARRY_MASK));
