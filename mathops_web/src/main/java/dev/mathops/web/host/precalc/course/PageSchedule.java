...
enum PageSchedule {
    ...
    static void doScheduleContent(final Cache cache, final CourseSiteLogic logic,
        ...
        if (!sortPaceOrder(paceReg)) {
            computePaceOrder(cache, paceReg);
        }
    ...
    static boolean sortPaceOrder(final List<RawStcourse> paceReg) {
        ...
        // See if all have a pace order specified
    ...
    /**
     * Computes the pace order for all registrations.  This could be called if the student adds a course after starting
     * other courses, so we don't want to reset orders that are already set.
     *
     * <p>
     * NOTE: On entry, {@code paceReg} is sorted in ascending order by course ID
     *
     * @param cache   the data cache
     * @param paceReg those registrations which contribute to pace
     * @throws SQLException if there is an error accessing the database
     */
    static void computePaceOrder(final Cache cache, final Collection<RawStcourse> paceReg) throws SQLException {

        Log.info("Computing pace order...");

        final int count = paceReg.size();

        // Clear any pace orders that are greater than the count
        for (final RawStcourse reg : paceReg) {
            if (reg.paceOrder != null && reg.paceOrder.intValue() > count) {
                reg.paceOrder = null;
            }
        }

        // Ensure there are no duplicate pace orders (if so, keep the order on the lower numbered course)
        int testOrder = 1;
        while (testOrder <= count) {
            boolean searching = true;
            for (final RawStcourse reg : paceReg) {
                if (reg.paceOrder != null && reg.paceOrder.intValue() == testOrder) {
                    if (searching) {
                        // Found the first one
                        searching = false;
                    } else {
                        // Found an additional one - fix it.
                        reg.paceOrder = null;
                    }
                }
            }
        ...
        int onOrder = 1;
        while (onOrder <= count) {
            // See of a course already has this order
            boolean searching = true;
            for (final RawStcourse reg : paceReg) {
                if (reg.paceOrder != null && reg.paceOrder.intValue() == onOrder) {
                    searching = false;
                    break;
                }
            }
            ...
            if (searching) {
                // There was no course with pace order "onOrder" - set the first course with no order to this
                for (final RawStcourse reg : paceReg) {
                    if (reg.paceOrder == null) {
                        SiteDataRegistration.updatePaceOrder(cache, reg, Integer.valueOf(onOrder));
                        reg.paceOrder = Integer.valueOf(onOrder);
                        break;
                    }
                }
            }
...
//    /**
//     * Generates a page that allows a user to place the set of courses which contribute towards their pace in order
//     when
...
//    /**
//     * Gathers and orders all incompletes in the pace registrations list, in order of deadline date. Must be called
//     from
...
//    /**
//     * Stores those registrations whose pace order has not yet been set in {@code tempMap}. Must be called from 
//     within a
//     * block synchronized on {@code tempMap}.
//     *
//     * @param paceReg the list of registrations that take part in the pace
...
//    /**
//     * Assigns pace orders to all records in the {@code tempMap} map, in the order given by the IDs list. Must be 
//     called
...
//    /**
//     * Generates a page that allows a user to place the set of courses which contribute towards their pace in order
//     * when there are two or more courses whose order is not determined and there is not just one with prerequisites
