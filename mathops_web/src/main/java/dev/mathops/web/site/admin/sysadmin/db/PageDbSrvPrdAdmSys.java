 ...
 */
public enum PageDbSrvPrdAdmSys {
    ;
        ...
        if (hostname == null || hostname.isEmpty()) {
            ...
            if (server == null) {
                ...
                if (selected == null) {
                    ...
                    htm.sDiv("boxed");
                    htm.sP("center")
                            .add("<small><strong>", product.name, " version ", version,
                                    "</strong> installed on <strong>", sanitized, "</strong></small>")
                            .eP().hr();

                    htm.add("<button class='nav6' onclick='pick(\"db_srv_prd_adm_dba.html?hostname=", hostname,
                            "&product=", selected.product.name, "&version=", selected.version, "\");'>",
                            "Database Admin", "</button>");

                    htm.add("<button class='nav6 selected' onclick='pick(\"db_srv_prd_adm_sys.html?hostname=",
                            hostname, "&product=", selected.product.name, "&version=", selected.version, "\");'>",
                            "System Admin", "</button>").hr();

                    // Sysadmin functions

                    htm.add("<button class='nav6' onclick='pick(\"db_srv_prd_adm_dba.html?hostname=",
                            hostname, "&product=", selected.product.name, "&version=", selected.version, "\");'>",
