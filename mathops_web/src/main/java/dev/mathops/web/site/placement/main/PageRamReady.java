...
enum PageRamReady {
    ...
    private static void doCheckMathPlacement(final Cache cache, final MathPlacementSite site,
        ...
        try {
            ...
            final RawStudent stu = RawStudentLogic.queryByInternalId(cache, pidmObj);
            if (stu == null) {
                sendErrorMessage(req, resp, "Math Placement status not available.");
            } else {
                final MathPlanPlacementStatus status = MathPlanLogic.getMathPlacementStatus(cache, stu.stuId);
