...
/**
 * Constructs the servlet WAR file that will serve the "course.[domain]" hostname.
 */
final class BuildCourseServletWar {
    ...
    /**
     * Constructs a new {@code BuildCourseServletWar}.
     */
    private BuildCourseServletWar() {
    ...
    private boolean buildRootJar() {
        ...
        final File webCourseClasses = new File(webClasses, "host/course");
        ...
        if (success) {
            try (final FileOutputStream out = new FileOutputStream(new File(wars, "COURSE_ROOT.jar"));
                ...
                addFiles(webRoot, webCourseClasses, jar);
                addFiles(webRoot, webSiteClasses, jar);

                jar.finish();
                Log.finest(Res.fmt(Res.JAR_DONE, "COURSE_ROOT"), CoreConstants.CRLF);
    ...
    private void buildRootWar() {
        ...
        try (final FileOutputStream out = new FileOutputStream(new File(warsDir, "COURSE_ROOT.war"));
            ...
            final File jarFile = new File(warsDir, "COURSE_ROOT.jar");
            war.putNextEntry(new ZipEntry("WEB-INF/lib/ROOT.jar"));
            war.write(FileLoader.loadFileAsBytes(jarFile, true));
            war.closeEntry();

            final File webFile = new File(webRoot, "dev/mathops/web/deploy/course_web.xml");
    ...
    public static void main(final String... args) {
        ...
        new BuildCourseServletWar().build();
