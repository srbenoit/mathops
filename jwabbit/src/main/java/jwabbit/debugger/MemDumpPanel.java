...
/**
 * A panel that displays the 16-bit addresses 0x0000 - 0xFFFF, converting each address to a wide address via the current
 * bank system and then querying the appropriate memory (RAM or flash).
 */
final class MemDumpPanel extends LoggedPanel implements ActionListener, ICalcStateListener {
    ...
    @Serial
    private static final long serialVersionUID = -5029499862490417431L;

    /** The information table. */
    private final JTable table;

    /** The data model for the table. */
    private final MemDumpTableModel tableModel;

    /**
     * Constructs a new {@code MemDumpPanel}.
     */
    MemDumpPanel() {
        ...
        this.tableModel = new MemDumpTableModel();
        ...
        final Font plainFont = new Font("Courier New", Font.PLAIN, this.table.getFont().getSize());
        this.table.setFont(plainFont);
        ...
        scroll.setPreferredSize(new Dimension(
                60 + 60 + 80 + 500 + scroll.getVerticalScrollBar().getPreferredSize().width + 2, 200));
    ...
    public void calcState(final Calc theCalc) {
        ...
        this.tableModel.setData(theCalc.getCPU().getMemoryContext());
     ...
     */
    private static final class MemDumpTableModel extends AbstractTableModel {

        /** Version number for serialization. */
        @Serial
        private static final long serialVersionUID = -2012945813609797671L;
        ...
        /**
         * Constructs a new {@code MemDumpTableModel}.
         */
        MemDumpTableModel() {
        ...
        /**
         * Sets the memory data by reading all bytes from a memory context into a local copy.
         *
         * @param memc the memory context from which to read memory
         */
        void setData(final MemoryContext memc) {

            final int dataLen = this.data.length;
            for (int i = 0; i < dataLen; ++i) {
                this.data[i] = memc.memRead(i);
            }
        }
