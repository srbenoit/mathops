...
/**
 * An index definition, as interpreted from the SQL file of an export.
 */
final class IndexDefinition {
    ...
    /**
     * Constructs a new {@code IndexDefinition}.
     *
     ...
     */
    IndexDefinition(final String sql, final int lineIndex, final String sqlFileName, final String dbName)
            throws IllegalArgumentException {

        final int onIndex = sql.indexOf(" on ");
        if (onIndex == -1) {
            final String lineStr = DataToImport.makeLineNumberText(lineIndex, sqlFileName);
            throw new IllegalArgumentException("Unable to find index name in index definition" + lineStr);
        }
        ...
        if (tableEnd == -1) {
            final String lineStr = DataToImport.makeLineNumberText(lineIndex, sqlFileName);
            throw new IllegalArgumentException("Unable to find table name in index definition" + lineStr);
        }
        ...
        if (fieldEnd == -1) {
            final String lineStr = DataToImport.makeLineNumberText(lineIndex, sqlFileName);
            throw new IllegalArgumentException("Unable to find field list in index definition" + lineStr);
        }
    ...
    String makeCreateSql(final String schemaName) {
        ...
        builder.add("CREATE INDEX ", this.indexName, " ON ", schemaName, ".", this.tableName, " (");
