...
/**
 * A map from named web context (host and path) or code context to the data profile that will be used to connect to
 * the database for that context. This can be used to select different profiles for various named contexts.
 *
 * <p>
...
public final class ContextMap {
    ...
    /** The singleton instance for each source XML file. */
