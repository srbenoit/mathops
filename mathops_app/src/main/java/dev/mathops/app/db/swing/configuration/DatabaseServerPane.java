...
/**
 * A pane that manages a single database server.  A server's configuration consists of a unique ID, the server type,
 * the schema it provides, the host and port on which it listens, an optional database name, and the username of the
 * DBA user.
 *
 * <p>
 * The "View" portion of this pane displays the attributes.  The "Control" portion allows these attributes to be
 * altered, and has a button to delete the server.  The {@code DatabaseServersPane} that presents a list of these panes
 * should provide a button to define a new server.
 */
public final class DatabaseServerPane extends JPanel implements IModelListener {
     ...
     */
    public DatabaseServerPane(final Model theModel, final String theServerId, final String theLoginId) {
