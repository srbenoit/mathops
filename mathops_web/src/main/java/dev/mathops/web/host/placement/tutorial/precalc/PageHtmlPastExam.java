...
enum PageHtmlPastExam {
     ...
     */
    static void startPastExam(final Cache cache, final PrecalcTutorialSite site, final ServletRequest req,
        ...
        if (AbstractSite.isFileParamInvalid(xml) || AbstractSite.isFileParamInvalid(upd)
            ...
            htm.addln("<form id='past_exam_form' action='update_past_exam.html' method='POST' onkeydown='if(event.keyCode === 13) {return false;}'>");
     ...
     * @throws SQLException if there is an error accessing the database
     */
    static void updatePastExam(final Cache cache, final PrecalcTutorialSite site, final ServletRequest req,
                               final HttpServletResponse resp, final ImmutableSessionInfo session)
            throws IOException, SQLException {
