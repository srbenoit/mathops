...
/**
 * A document object that supports a boolean choice. The choice will be presented to the student as a checkbox, which
 * can either be checked or unchecked, or can be in an unanswered state.
 *
 * <p>
 ...
 */
public final class DocInputCheckbox extends AbstractDocInput {

    /** Version number for serialization. */
    @Serial
    private static final long serialVersionUID = 3302997213440321776L;
     ...
     */
    @Override
    public DocInputCheckbox deepCopy() {

        final DocInputCheckbox copy = new DocInputCheckbox(getName(), this.value);
        ...
        synchronized (this) {
            ...
            if (ctx != null) {
                ...
                if (param != null) {
                    ...
                    if (theValue instanceof final Long valueAsLong) {
                        final long existingValue = valueAsLong.longValue();
                        isSelected = (existingValue & this.value) != 0L;
                    }
                }
            }
    ...
    void selectChoice() {
        ...
        synchronized (this) {
            ...
            if (getEvalContext() != null) {
                ...
                if (param instanceof VariableInputInteger) {

                    // If value is not changing, do nothing.
                    if (param.getValue() instanceof final Long valueAsLong) {
                        final long existingValue = valueAsLong.longValue();

                        if ((existingValue & this.value) == this.value) {
                            return;
                        }

                        final long newValue = existingValue | this.value;
                        storeValue(Long.valueOf(newValue));
                    } else {
                    ...
                    Log.warning("Parameter associated with DocInputCheckbox is not InputInteger");
                }
            }
        }
    }
        ...
        synchronized (this) {
            if (getEvalContext() != null) {
                ...
                if (param instanceof VariableInputInteger) {
                    ...
                    if (param.getValue() instanceof final Long valueAsLong) {
                        final long existingValue = valueAsLong.longValue();
                        final boolean isSelected = (existingValue & this.value) != 0L;
                        final long newValue = isSelected ? (existingValue & (~this.value))
    ...
    public String toString() {
        ...
        if (isChoiceSelected()) {
            builder.add("{", getName(), "}=", Long.toString(this.value));
    ...
    public void paintComponent(final Graphics grx, final ELayoutMode mathMode) {
        ...
        if ((SHADOW_COLOR != null) && (grx instanceof Graphics2D)) {
            ...
            grx.drawRect(INSET_LEFT + 1, INSET_TOP + 1, size, size);
        }
        ...
        if (isChoiceSelected()) {
            ...
            grx.fillRect(x, y, size - 9, size - 9);
        }
    ...
    public DocInputCheckboxInst createInstance(final EvalContext evalContext) {
        ...
        return new DocInputCheckboxInst(objStyle, null, getName(), getEnabledVarName(), getEnabledVarValue(),
                this.value);
    }
    ...
    public void toXml(final HtmlBuilder xml, final int indent) {
        ...
        xml.add(" value='", Long.toString(this.value), "' selected='", Boolean.toString(isChoiceSelected()), "'>");
    ...
    public boolean setValue(final String theValue) {
        ...
        if (theValue.startsWith(theTag)) {
            ...
            if (toParse.isBlank()) {
                ...
                try {
                    final long valueToSet = Long.parseLong(toParse);

                    if ((valueToSet & this.value) == this.value) {
                        selectChoice();
                    }

                    result = true;
                } catch (final NumberFormatException e) {
                    Log.warning(Res.fmt(Res.UNABLE_TO_PARSE, theValue), e);
                    result = false;
                }
            }
        } else {
    ...
    public int hashCode() {
        ...
        return inputInnerHashCode() + (int) this.value;
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (obj == this) {
            equal = true;
        } else if (obj instanceof final DocInputCheckbox checkbox) {
            equal = inputInnerEquals(checkbox) && this.value == checkbox.value;
        } else {
            equal = false;
        }
