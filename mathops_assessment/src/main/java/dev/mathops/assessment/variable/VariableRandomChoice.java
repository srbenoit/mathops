...
public final class VariableRandomChoice extends AbstractFormattableVariable implements IExcludableVariable {
    ...
    public void whyNotEqual(final AbstractVariable obj, final int indent) {
        ...
        if (obj instanceof final VariableRandomChoice var) {

            if (!Objects.equals(this.exclude, var.exclude)) {
                if (this.exclude == null || var.exclude == null) {
                    Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (exclude: " + this.exclude + "!="
                            + var.exclude + ")");
                } else {
                    ...
                    if (excludeLen == var.exclude.length) {
                        for (int i = 0; i < excludeLen; ++i) {
                            ...
                            if (!Objects.equals(f1, f2)) {
                                if (f1 == null || f2 == null) {
                                    Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (exclude[",
                        ...
                        Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (exclude.length: "
                                + Integer.valueOf(excludeLen) + "!=" + Integer.valueOf(var.exclude.length) + ")");
                    }
                }
            }

            if (!Objects.equals(this.chooseFrom, var.chooseFrom)) {
                if (this.chooseFrom == null || var.chooseFrom == null) {
                    Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (chooseFrom: " + this.chooseFrom
                            + "!=" + var.chooseFrom + ")");
                } else {
                    final int chooseLength = this.chooseFrom.length;
                    if (chooseLength == var.chooseFrom.length) {
                        for (int i = 0; i < chooseLength; ++i) {
                            final Formula f1 = this.chooseFrom[i];
                            final Formula f2 = var.chooseFrom[i];

                            if (!Objects.equals(f1, f2)) {
                                if (f1 == null || f2 == null) {
                                    Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (chooseFrom[",
                                            Integer.toString(i), "]: ", f1, "!=", f2, ")");
                                } else {
                                    Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (chooseFrom[",
                                            Integer.toString(i), "])");
                        ...
                        Log.info(makeIndent(indent), "UNEQUAL VariableRandomChoice (chooseFrom.length: "
                                + Integer.valueOf(chooseLength) + "!="
                                + Integer.valueOf(var.chooseFrom.length) + ")");
                    }
                }
            }
        }
