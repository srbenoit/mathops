...
public final class ConstRealVector extends AbstractFormulaObject implements IEditableFormulaObject {
    ...
    public void whyNotEqual(final Object other, final int indent) {
        ...
        if (other instanceof final ConstRealVector obj) {
            if (!this.value.equals(obj.value)) {
                Log.info(makeIndent(indent), "UNEQUAL ConstRealVector (value: ", this.value, "!=", obj.value, ")");
            }
        } else {
            Log.info(makeIndent(indent), "UNEQUAL ConstRealVector because other is ", other.getClass().getName());
        }
    }
