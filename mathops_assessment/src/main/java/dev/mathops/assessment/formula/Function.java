...
public final class Function extends AbstractFormulaContainer implements IEditableFormulaObject {
    ...
    public void whyNotEqual(final Object other, final int indent) {
        ...
        if (other instanceof final Function obj) {
            innerWhyNotEqual(obj, indent);

            if (!Objects.equals(this.function, obj.function)) {
                Log.info(makeIndent(indent), "UNEQUAL Function (op: ", this.function.name, "!=",
                        obj.function.name, ")");
            }
        } else {
            Log.info(makeIndent(indent), "UNEQUAL Function because other is ", other.getClass().getName());
        }
    }
