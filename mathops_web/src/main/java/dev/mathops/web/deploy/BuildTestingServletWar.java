...
/**
 * Constructs the servlet WAR file that will serve the "testing.[domain]" hostname.
 */
final class BuildTestingServletWar {
     ...
     */
    private BuildTestingServletWar() {
    ...
    private boolean buildRootJar() {
        ...
        final File webTestingClasses = new File(webClasses, "host/testing");
        ...
        if (success) {
            try (final FileOutputStream out = new FileOutputStream(new File(wars, "TESTING_ROOT.jar"));
                ...
                addFiles(webRoot, webTestingClasses, jar);
                addFiles(webRoot, webSiteClasses, jar);

                jar.finish();
                Log.finest(Res.fmt(Res.JAR_DONE, "TESTING_ROOT"), CoreConstants.CRLF);
    ...
    private void buildRootWar() {
        ...
        try (final FileOutputStream out = new FileOutputStream(new File(warsDir, "TESTING_ROOT.war"));
            ...
            final File jarFile = new File(warsDir, "TESTING_ROOT.jar");
            war.putNextEntry(new ZipEntry("WEB-INF/lib/ROOT.jar"));
            war.write(FileLoader.loadFileAsBytes(jarFile, true));
            war.closeEntry();

            final File webFile = new File(webRoot, "dev/mathops/web/deploy/testing_web.xml");
    ...
    public static void main(final String... args) {
        ...
        new BuildTestingServletWar().build();
