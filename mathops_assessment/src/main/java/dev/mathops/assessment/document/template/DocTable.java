...
//        }
    ...
    public void whyNotEqual(final Object other, final int indent) {
        ...
        if (other instanceof final DocTable obj) {
            innerWhyNotEqual(obj, indent);

            if ((this.objectData == null) || (obj.objectData == null)) {
                if (obj.objectData != null) {
                    Log.info(makeIndent(indent), "UNEQUAL DocTable (objectData: null!=", obj.objectData, ")");
                }
            } else if (this.objectData.length != obj.objectData.length) {
                Log.info(makeIndent(indent), "UNEQUAL DocTable (objectData length: " + this.objectData.length + "!="
                        + obj.objectData.length + ")");
            } else {
                final int len = this.objectData.length;
                for (int i = 0; i < len; ++i) {
                    final AbstractDocObjectTemplate[] row1 = this.objectData[i];
                    final AbstractDocObjectTemplate[] row2 = obj.objectData[i];

                    if (row1 == null || row2 == null) {
                        Log.info(makeIndent(indent), "UNEQUAL DocTable (objectData row " + i + ": ", row1, "!=", row2,
                                ")");
                    } else if (row1.length != row2.length) {
                        Log.info(makeIndent(indent), "UNEQUAL DocTable (objectData row " + i + " length: "
                                + row1.length + "!=" + row2.length + ")");
                    } else {
                        final int rowLen = row1.length;
                        for (int j = 0; j < rowLen; ++j) {
                            final AbstractDocObjectTemplate o1 = row1[j];
                            final AbstractDocObjectTemplate o2 = row2[j];
                            if (!Objects.equals(o1, o2)) {

                                if (o1 == null || o2 == null) {
                                    Log.info(makeIndent(indent), "UNEQUAL DocTable (objectData row " + i + " col "
                                            + j + ": " + o1 + "!=" + o2 + ")");
                                } else {
                                    Log.info(makeIndent(indent), "UNEQUAL DocTable (objectData row " + i + " col "
                                            + j + "...)");
            ...
            if (this.spacing != obj.spacing) {
                Log.info(makeIndent(indent), "UNEQUAL DocTable (spacing: " + this.spacing + "!=" + obj.spacing + ")");
            }

            if (this.justification != obj.justification) {
                Log.info(makeIndent(indent), "UNEQUAL DocTable (justification: " + this.justification + "!="
                        + obj.justification + ")");
            }

            if (!Objects.equals(this.backgroundColorName, obj.backgroundColorName)) {
                Log.info(makeIndent(indent), "UNEQUAL DocTable (backgroundColorName: ", this.backgroundColorName,
                        "!=", obj.backgroundColorName, ")");
            }
        } else {
            Log.info(makeIndent(indent), "UNEQUAL DocTable because other is ", other.getClass().getName());
        }
    }
}
