...
/**
 * A container for a single Real-valued object that can generate a Real-valued {@code Function}. This is applicable to
 * the "cos", "sin", "tan", "acos", "asin", "atan", "exp", "log", "abs", "toDeg", "toRad", "sqrt", and "cbrt"
 * functions.
 */
public final class FEFunction extends AbstractFEObject {

    /** The function. */
    private final EFunction fxn;
    ...
    /**
     * Constructs a new {@code FEFunctionRealR}.
     *
     * @param theFontSize the font size for the component
     * @param theFxn      the operator
     */
    public FEFunction(final int theFontSize, final EFunction theFxn) {
     ...
     * @return the argument
     */
    public AbstractFEObject getArg() {

        return this.arg;
    }

    /**
     * Gets the total number of "cursor steps" in the object and its descendants. There is a "fxn(" opening tag,
     * followed by the argument, then a ")" closing tag.
     *
    ...
    public void recomputeCurrentType() {
        ...
        if (this.arg == null) {
            ...
            switch (this.fxn) {
                ...
                case SRAD2:
                case SRAD3:
                    setCurrentType(EType.INTEGER);
                    possible.add(EType.INTEGER);
                    break;

                case LCASE:
                case UCASE:
                    setCurrentType(EType.STRING);
                    possible.add(EType.STRING);
                    break;
    ...
    public boolean replaceChild(final AbstractFEObject currentChild,  final AbstractFEObject newChild) {
        ...
        if (this.arg == null) {
            ...
            if (newChild == null) {
                ...
                final EnumSet<EType> allowedArgs = getAllowedArgumentTypes();
                ...
                if (newType == null) {
                    ...
                    Log.warning("Attempt to add ", newType, " type child to function when not allowed");
                    result = false;
                }
            }
        } else {
    ...
    /**
     * Determines the argument types allowed based on the function.
     *
    ...
    public void indicateValidModifications(final FECursor fECursor,
        ...
        if (this.arg == null) {
            ...
            if (allowedArgs.contains(EType.STRING)) {
    ...
    public void processChar(final FECursor fECursor, final char ch) {
        ...
        Log.info("Function container processing '", chStr, "' at " + cursorPos);
        ...
        if (cursorPos > 0 && cursorPos < getNumCursorSteps()) {
            if (this.arg == null) {
                ...
                if ((int) ch >= '0' && (int) ch <= '9') {
                    if (allowedArgs.contains(EType.REAL) || allowedArgs.contains(EType.INTEGER)) {
                    ...
                    if (allowedArgs.contains(EType.REAL_VECTOR) || allowedArgs.contains(EType.INTEGER_VECTOR)) {
                        ++fECursor.cursorPosition;
                    ...
                    testAllowed.clear();
                    testAllowed.addAll(allowedArgs);
                    setArg(test, true);
                    ...
                    if (((int) ch == 0x7f && cursorPos < getNumCursorSteps()) && (this.arg == null)) {
                        getParent().replaceChild(this, null);
                    }
            } else {
    ...
    public String processInsert(final FECursor fECursor, final AbstractFEObject toInsert) {
        ...
        if (this.arg == null) {
            ...
            if (fECursor.cursorPosition > first && fECursor.cursorPosition < last) {
                ...
                error = "Unable to insert into function outside the parentheses.";
            }
        }
    ...
    public void layout(final Graphics2D g2d) {
        ...
        this.openParen = new RenderedBox(this.fxn.name + "(");
        ...
        this.closeParen.getOrigin().setLocation(x, 0);
    ...
    public void emitDiagnostics(final HtmlBuilder builder, final int indent) {
        ...
        builder.addln("Function: (", this.fxn.name, ")");
     ...
     */
    @Override
    public FEFunction duplicate() {

        final int fontSize = getFontSize();
        final FEFunction dup = new FEFunction(fontSize, this.fxn);
