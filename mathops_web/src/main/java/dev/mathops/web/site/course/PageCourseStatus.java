...
enum PageCourseStatus {
    ...
    static void doGet(final Cache cache, final CourseSite site, final ServletRequest req,
        ...
        if (AbstractSite.isParamInvalid(course)) {
            ...
            PageError.doGet(cache, site, req, resp, session, "No course ID provided for status display");
        } else if (course == null) {
            PageError.doGet(cache, site, req, resp, session, "No course ID provided for status display");
        } else {
            ...
            doStatus(cache, site, session, logic, course, htm);
     ...
     * @throws SQLException if there is an error accessing the database
     */
    private static void doStatus(final Cache cache, final CourseSite site,
