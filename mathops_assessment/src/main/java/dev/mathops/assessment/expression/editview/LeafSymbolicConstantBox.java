package dev.mathops.assessment.expression.editview;

import dev.mathops.assessment.expression.editmodel.ESymbolicConstant;
import dev.mathops.assessment.expression.editmodel.ExprLeafSymbolicConstant;
import dev.mathops.commons.builder.SimpleBuilder;

import java.awt.Color;
import java.awt.Font;
import java.awt.FontMetrics;
import java.awt.Graphics2D;
import java.awt.font.GlyphVector;
import java.awt.geom.Rectangle2D;

/**
 * A laid out {@code ExprLeafOperator}.
 */
public final class LeafSymbolicConstantBox extends AbstractObjectBox {

    /** The source object. */
    private final ExprLeafSymbolicConstant source;

    /** The font. */
    private final Font font;

    /** The color. */
    private final Color color;

    /**
     * Constructs a new {@code LeafSymbolicConstantBox}.
     *
     * @param theSource the source object
     * @param currentFontSize the current font size
     */
    LeafSymbolicConstantBox(final ExprLeafSymbolicConstant theSource, final float currentFontSize) {

        super();

        this.source = theSource;

        this.font = Fonts.math.deriveFont(currentFontSize);
        this.color = Color.BLACK;
        setAxisCenter(this.font);

        final ESymbolicConstant value = theSource.getValue();

        final FontMetrics metrics = Fonts.g2d.getFontMetrics(this.font);
        final int w = metrics.stringWidth(value.str);
        setWidth(w);

        final GlyphVector gvector = this.font.createGlyphVector(Fonts.frc, value.str);
        final Rectangle2D bounds = gvector.getVisualBounds();

        // Note: positive Y is down, so max is descent, min is ascent
        final double maxY = bounds.getMaxY();
        final double minY = bounds.getMinY();

        final int theBottom = (int)(-Math.ceil(maxY));
        final int theTop = (int)(-Math.floor(minY));
        setTopBottom(theTop, theBottom);
    }

    /**
     * Paints the contents of the box
     *
     * @param g2d the {@code Graphics2D} to which to draw
     */
    @Override
    public void paint(final Graphics2D g2d) {

        final ESymbolicConstant value = this.source.getValue();

        g2d.setColor(this.color);
        g2d.setFont(this.font);
        g2d.drawString(value.str, this.x, this.y);
    }

    /**
     * Generates the diagnostic string representation of the object.
     *
     * @return the string representation
     */
    @Override
    public String toString() {

        return SimpleBuilder.concat("LeafSymbolicConstantBox{source=", this.source, ", font=", this.font, ", color=",
                this.color, "}");
    }
}
