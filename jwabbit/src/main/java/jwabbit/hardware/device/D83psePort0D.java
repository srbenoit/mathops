...
/**
 * The port 13 (0x0D) device for the TI 83pse hardware.
 */
public final class D83psePort0D extends AbstractDevice {

    /** The LinkAssist to which this device interfaces. */
    private LinkAssist linkAssist;

    /**
     * Constructs a new {@code Device83psePort0D}.
     *
     * @param theDevIndex the index at which this device is installed
     */
    public D83psePort0D(final int theDevIndex) {
    ...
    /**
     * WABBITEMU SOURCE: hardware/83psehw.c, "port0D_83pse" function.
     *
     * @param cpu the CPU
     */
    @Override
    public void runCode(final CPU cpu) {
        ...
        if (cpu.isInput()) {
            cpu.setBus(this.linkAssist.getOut());
            this.linkAssist.setReady(false);
            cpu.setInput(false);
        } else if (cpu.isOutput()) {
            this.linkAssist.setOut(cpu.getBus());
            this.linkAssist.setReady(false);
            this.linkAssist.setSending(true);
