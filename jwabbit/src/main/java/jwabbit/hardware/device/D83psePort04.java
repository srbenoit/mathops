...
/**
 * The port 4 device for the TI 83pse hardware.
 */
public final class D83psePort04 extends AbstractDevice {

    /** The STDINT to which this device interfaces. */
    private STDINT stdint;

    /**
     * Constructs a new {@code Device83psePort04}.
     *
     * @param theDevIndex the index at which this device is installed
     */
    public D83psePort04(final int theDevIndex) {
    ...
    /**
     * WABBITEMU SOURCE: hardware/83psehw.c, "port4_83pse" function.
     *
     * @param cpu the CPU
     */
    @Override
    public void runCode(final CPU cpu) {
        ...
        final XTAL xtal = cpu.getPIOContext().getSeAux().getXtal();
        ...
        if (cpu.isInput()) {
            ...
            if ((this.stdint.getIntactive() & (1 << 1)) != 0
                    && (cpu.getTimerContext().getElapsed() - this.stdint.getLastchk1()) > this.stdint
                    .getTimermax1()) {
                result += 1 << 1;
            }
            if ((this.stdint.getIntactive() & (1 << 2)) != 0
                    && (cpu.getTimerContext().getElapsed() - this.stdint.getLastchk2()) > this.stdint
                    .getTimermax2()) {
                result += 1 << 2;
            }
            ...
            if (xtal.getTimer(0).isUnderflow()) {
                result += 1 << 5;
            }
            if (xtal.getTimer(1).isUnderflow()) {
                result += 1 << 6;
            }
            if (xtal.getTimer(2).isUnderflow()) {
                result += 1 << 7;
            }
