...
/**
 * Constructs the servlet WAR file that will serve the "nibbler.[domain]" hostname.
 */
final class BuildNibblerServletWar {
     ...
     */
    private BuildNibblerServletWar() {
    ...
    private boolean buildRootJar() {
        ...
        final File webNibblerClasses = new File(webClasses, "host/nibbler");
        ...
        if (success) {
            try (final FileOutputStream out = new FileOutputStream(new File(wars, "NIBBLER_ROOT.jar"));
                ...
                addFiles(webRoot, webNibblerClasses, jar);
                addFiles(webRoot, webSiteClasses, jar);

                jar.finish();
                Log.finest(Res.fmt(Res.JAR_DONE, "NIBBLER_ROOT"), CoreConstants.CRLF);
    ...
    private void buildRootWar() {
        ...
        try (final FileOutputStream out = new FileOutputStream(new File(warsDir, "NIBBLER_ROOT.war"));
            ...
            final File jarFile = new File(warsDir, "NIBBLER_ROOT.jar");
            war.putNextEntry(new ZipEntry("WEB-INF/lib/ROOT.jar"));
            war.write(FileLoader.loadFileAsBytes(jarFile, true));
            war.closeEntry();

            final File webFile = new File(webRoot, "dev/mathops/web/deploy/nibbler_web.xml");
    ...
    public static void main(final String... args) {
        ...
        new BuildNibblerServletWar().build();
