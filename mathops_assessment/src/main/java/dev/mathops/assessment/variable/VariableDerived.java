...
public final class VariableDerived extends AbstractFormattableVariable
    ...
    public void whyNotEqual(final AbstractVariable obj, final int indent) {
        ...
        if (obj instanceof final VariableDerived var) {

            if (!Objects.equals(this.min, var.min)) {
                if (this.min == null || var.min == null) {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableRandomInt (min: " + this.min
                            + "!=" + var.min + ")");
                } else {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableDerived (min)");
                }
            }

            if (!Objects.equals(this.max, var.max)) {
                if (this.max == null || var.max == null) {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableRandomInt (max: " + this.max
                            + "!=" + var.max + ")");
                } else {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableDerived (max)");
                }
            }

            if (!Objects.equals(this.formula, var.formula)) {
                if (this.formula == null || var.formula == null) {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableDerived (formula: " + this.formula
                            + "!=" + var.formula + ")");
                } else {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableDerived (formula)");
                    this.formula.whyNotEqual(var.formula, indent + 1);
                }
            }

            if (!Objects.equals(this.exclude, var.exclude)) {
                if (this.exclude == null || var.exclude == null) {
                    Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableDerived (exclude: "
                            + this.exclude + "!=" + var.exclude + ")");
                } else {
                    ...
                    if (excludeLen == var.exclude.length) {
                        ...
                        Log.info(AbstractXmlObject.makeIndent(indent), "UNEQUAL VariableDerived (exclude.length: "
                                + Integer.valueOf(excludeLen) + "!=" + Integer.valueOf(var.exclude.length) + ")");
                    }
                }
            }
        }
