...
import dev.mathops.db.rawrecord.RawMpscorequeue;
import dev.mathops.db.rawrecord.RawStchallenge;
import dev.mathops.db.rawrecord.RawStchallengeqa;
 ...
 * tutorial web sites can be directed to use DEV to keep those sites open during the rollover process. This requires
 * extending the dates (in DEV) of the ending term and the "csection" rows for tutorials so testing can still take
 * place.
 *
 * <p>
 * Once rollover is complete and PROD is ready for the following term, all web sites can be turned off, and this batch
 * run to copy all placement and tutorial records (and any Math Plan submissions) that were submitted to DEV over to
 * PROD. At the same time, the web sites should be redirected to PROD, and the web sites restarted.
 *
 * <p>
...
final class TransferPlacementDevtoProd {
    ...
    /** The database profile through which to access the PROD database. */
    private final DbProfile prodProfile;

    /** The database profile through which to access the DEV database. */
    private final DbProfile devProfile;
    ...
    private TransferPlacementDevtoProd(final LocalDateTime theSince) {
        ...
        this.prodProfile = map.getCodeProfile(Contexts.ADMPROD_PATH);
        this.devProfile = map.getCodeProfile(Contexts.ADMDEV_PATH);
    ...
    private String execute() {
        ...
        if (this.prodProfile == null) {
            ...
            final DbContext prodCtx = this.prodProfile.getDbContext(ESchemaUse.PRIMARY);
            final DbContext devCtx = this.devProfile.getDbContext(ESchemaUse.PRIMARY);

            try {
                final DbConnection prodConn = prodCtx.checkOutConnection();

                try {
                    final DbConnection devConn = devCtx.checkOutConnection();
                    ...
                    try {
                        ...
                        devCtx.checkInConnection(devConn);
                    }
                } finally {
                    prodCtx.checkInConnection(prodConn);
                }
    ...
    public static void main(final String... args) {
        ...
        // SM23: Cutover to DEV happened on August 7 at 6:45 AM
        final LocalDateTime since = LocalDateTime.of(2023, 8, 7, 6, 45);

        final TransferPlacementDevtoProd job = new TransferPlacementDevtoProd(since);
