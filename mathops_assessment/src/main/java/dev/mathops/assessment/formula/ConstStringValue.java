...
public final class ConstStringValue extends AbstractFormulaObject implements IEditableFormulaObject {
    ...
    public void whyNotEqual(final Object other, final int indent) {
        ...
        if (other instanceof final ConstStringValue obj) {
            if (!this.value.equals(obj.value)) {
                Log.info(makeIndent(indent), "UNEQUAL ConstRealValue (value: ", this.value, "!=", obj.value, ")");
            }
        } else {
            Log.info(makeIndent(indent), "UNEQUAL ConstRealValue because other is ", other.getClass().getName());
        }
    }
