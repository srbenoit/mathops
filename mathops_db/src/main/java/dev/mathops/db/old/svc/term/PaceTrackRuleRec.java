...
public final class PaceTrackRuleRec extends RecBase implements Comparable<PaceTrackRuleRec> {
     ...
     */
    public static PaceTrackRuleRec parse(final String toParse) throws IllegalArgumentException {

        final ParsedRecFields parsed = new ParsedRecFields(7, toParse);

        final String theSubterm = parsed.has(FLD_SUBTERM) ? parsed.get(FLD_SUBTERM) : null;
        final Integer thePace = parsed.has(FLD_PACE) ? Integer.valueOf(parsed.get(FLD_PACE)) : null;
        final String thePaceTrack = parsed.has(FLD_PACE_TRACK) ? parsed.get(FLD_PACE_TRACK) : null;
        final String theCriteria = parsed.has(FLD_CRITERIA) ? parsed.get(FLD_CRITERIA) : null;

        return new PaceTrackRuleRec(theSubterm, thePace, thePaceTrack, theCriteria);
    }
