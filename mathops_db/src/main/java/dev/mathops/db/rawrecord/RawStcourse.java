...
public final class RawStcourse extends RawTermRecordBase implements Comparable<RawStcourse> {
    ...
    /**
     * Constructs a new {@code RawStcourse} by parsing the string format generated by {@code serializedString}.
     *
     * @param toParse the string to parse
     * @return the parsed object
     * @throws IllegalArgumentException if the string cannot be parsed
     */
    public static RawStcourse parse(final String toParse) {
    ...
    protected void setField(final String name, final String value) throws IllegalArgumentException {
        ...
        if (FLD_TERM.equals(name)) {
            ...
            this.sect = value;
        } else if (FLD_PACE_ORDER.equals(name)) {
            this.paceOrder = Integer.valueOf(value);
        } else if (FLD_OPEN_STATUS.equals(name)) {
            this.openStatus = value;
        } else if (FLD_GRADING_OPTION.equals(name)) {
            this.gradingOption = value;
        } else if (FLD_COMPLETED.equals(name)) {
            this.completed = value;
        } else if (FLD_SCORE.equals(name)) {
            this.score = Integer.valueOf(value);
        } else if (FLD_COURSE_GRADE.equals(name)) {
            this.courseGrade = value;
        } else if (FLD_PREREQ_SATIS.equals(name)) {
            this.prereqSatis = value;
        } else if (FLD_INIT_CLASS_ROLL.equals(name)) {
            this.initClassRoll = value;
        } else if (FLD_STU_PROVIDED.equals(name)) {
            this.stuProvided = value;
        } else if (FLD_FINAL_CLASS_ROLL.equals(name)) {
            this.finalClassRoll = value;
        } else if (FLD_EXAM_PLACED.equals(name)) {
            this.examPlaced = value;
        } else if (FLD_ZERO_UNIT.equals(name)) {
            this.zeroUnit = Integer.valueOf(value);
        } else if (FLD_TIMEOUT_FACTOR.equals(name)) {
            this.timeoutFactor = Float.valueOf(value);
        } else if (FLD_FORFEIT_I.equals(name)) {
            this.forfeitI = value;
        } else if (FLD_I_IN_PROGRESS.equals(name)) {
            this.iInProgress = value;
        } else if (FLD_I_COUNTED.equals(name)) {
            this.iCounted = value;
        } else if (FLD_CTRL_TEST.equals(name)) {
            this.ctrlTest = value;
        } else if (FLD_DEFERRED_F_DT.equals(name)) {
            this.deferredFDt = LocalDate.parse(value);
        } else if (FLD_BYPASS_TIMEOUT.equals(name)) {
            this.bypassTimeout = Integer.valueOf(value);
        } else if (FLD_INSTRN_TYPE.equals(name)) {
            this.instrnType = value;
        } else if (FLD_REGISTRATION_STATUS.equals(name)) {
            this.registrationStatus = value;
        } else if (FLD_LAST_CLASS_ROLL_DT.equals(name)) {
            this.lastClassRollDt = LocalDate.parse(value);
        } else if (FLD_I_TERM.equals(name)) {
            this.iTermKey = TermKey.parseLongString(value);
        } else if (FLD_I_DEADLINE_DT.equals(name)) {
            this.iDeadlineDt = LocalDate.parse(value);
        }
    }
    ...
    /**
     * Extracts an "stcourse" record from a result set.
     *
    ...
    public int hashCode() {
        ...
        return EqualityTests.objectHashCode(this.termKey)
                ...
                + EqualityTests.objectHashCode(this.openStatus)
                + EqualityTests.objectHashCode(this.gradingOption)
                + EqualityTests.objectHashCode(this.completed)
                + EqualityTests.objectHashCode(this.score)
                + EqualityTests.objectHashCode(this.courseGrade)
                + EqualityTests.objectHashCode(this.prereqSatis)
                + EqualityTests.objectHashCode(this.initClassRoll)
                + EqualityTests.objectHashCode(this.stuProvided)
                + EqualityTests.objectHashCode(this.finalClassRoll)
                + EqualityTests.objectHashCode(this.examPlaced)
                + EqualityTests.objectHashCode(this.zeroUnit)
                + EqualityTests.objectHashCode(this.timeoutFactor)
                + EqualityTests.objectHashCode(this.forfeitI)
                + EqualityTests.objectHashCode(this.iInProgress)
                + EqualityTests.objectHashCode(this.iCounted)
                + EqualityTests.objectHashCode(this.ctrlTest)
                + EqualityTests.objectHashCode(this.deferredFDt)
                + EqualityTests.objectHashCode(this.bypassTimeout)
                + EqualityTests.objectHashCode(this.instrnType)
                + EqualityTests.objectHashCode(this.registrationStatus)
                + EqualityTests.objectHashCode(this.lastClassRollDt)
                + EqualityTests.objectHashCode(this.iTermKey)
                + EqualityTests.objectHashCode(this.iDeadlineDt);
    }
